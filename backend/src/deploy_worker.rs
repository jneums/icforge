...
async fn execute_deploy(
    ...
    if tokio::fs::metadata(&mops_toml_path).await.is_ok() {
        ...
        // Remove stale mops.lock to avoid integrity check failures.
        // The lockfile may have been generated by a different mops version
        // and the deploy environment is ephemeral — lockfile adds no value here.
        let mops_lock_path = format!("{work_dir}/mops.lock");
        let _ = tokio::fs::remove_file(&mops_lock_path).await;

        // `mops install` handles both package installation and toolchain setup
        // (downloads moc compiler version specified in [toolchain] section of mops.toml)
