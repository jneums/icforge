...
pub async fn deploy_logs(
    ...
    let log_entries: Vec<DeployLogEntry> = logs.into_iter()
...
pub async fn deploy_logs_stream(
    ...
    let replay_entries: Vec<DeployLogEntry> = existing_logs
        .into_iter()
        .map(|l| DeployLogEntry {
            level: l.level,
            message: l.message,
    ...
    // Subscribe to broadcast channel BEFORE reading DB logs.
    // This ensures we don't miss events between DB read and subscribe.
    let rx = state
        .log_channels
        .get(&deploy_id)
